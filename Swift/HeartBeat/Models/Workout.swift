...
class Workout {
    ...
    func compareWith(workoutObject: WorkoutObject) -> Bool {
        if recordID?.recordName == workoutObject.recordName {
            return true
        }
        return false
    }
    
    func workoutObject() -> WorkoutObject {
        let w = WorkoutObject()
        w.arrayBeatsPerMinute = arrayBeatsPerMinute
        w.beatsPerMinuteAverage = NSNumber(integer: beatsPerMinuteAverage!)
        w.caloriesBurned = NSNumber(integer: caloriesBurned!)
        w.endTime = endTime
        w.secondsElapsed = NSNumber(integer: secondsElapsed!)
        w.startTime = startTime
        w.workoutType = workoutType
        w.recordName = recordID?.recordName
